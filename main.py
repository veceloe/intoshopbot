...
async def handle_button_click(message: types.Message):
    ...
    if message.text == "Информация о стоимости доставки":
        answer = "Есть доставка до корейского адреса, ее стоимость уже входит в пост с оплатой разбора. Также есть " \
                 "доставка из Кореи до РФ к админкам, в среднем стоимость за одну карту составляет 80-120 рублей/ за " \
                 "альбом 400-1000 рублей. Информацию по поводу другого стаффа уточняйте у админок. После упаковки " \
    ...
    # Обработка нажатия кнопки "Как высчитать цену"
        ...
        answer = (
            "Чтобы узнать цену карты, нужно умножить 105 на цифру после запятой (0.2 = 105 × 2 = 210 рублей)\n"
            "Если карта стоит 1.0 и больше, то нужно умножить все число без запятой на 105\n"
            "(1.3 = 13×105 = 1365 рублей)\n"
            "Если после запятой две цифры, то нужно умножить число сначала на 10, затем на 105 (0.15 = 0.15 × 10 = 1.5,"
            "затем 1.5 × 105 = 157.5 рублей)\n"
